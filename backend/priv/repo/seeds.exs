...
unless System.get_env("CANOPY_SEED_DEMO") == "true" do
  ...
  unless Repo.exists?(
           from p in Project,
             where: p.workspace_id == ^workspace.id and p.name == "Canopy Platform"
         ) do
    Repo.insert!(%Project{
  ...
  unless Repo.exists?(
           from p in Project,
             where: p.workspace_id == ^workspace.id and p.name == "Infrastructure"
         ) do
    Repo.insert!(%Project{
      name: "Infrastructure",
  ...
  IO.puts(
    "    4 goals: Launch MVP, Implement Adapter System (child), Setup CI/CD, Security Audit"
  )
  ...
  unless Repo.exists?(
           from b in BudgetPolicy,
             where: b.scope_type == "agent" and b.scope_id == ^orchestrator.id
         ) do
    Repo.insert!(
  ...
  for attrs <- integrations do
    unless Repo.exists?(
             from i in Integration,
               where: i.workspace_id == ^workspace.id and i.slug == ^attrs.slug
           ) do
      Repo.insert!(
